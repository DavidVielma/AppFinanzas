...
export function ColorPicker({ name = "color", value, defaultValue, onChange, presets = [], compact = false }) {
  ...
  return (
    <div className={`color-picker ${compact ? "compact" : ""}`}>
      ...
      <input className="color-hex-input" value={currentColor} onChange={(event) => setColor(event.target.value)} aria-label="Color hexadecimal" />
